...
- Website: `https://lsp-readiness-check.sociobot.in/?demo=1` (the one-click sandbox URL), `https://lsp-readiness-check.sociobot.in/demo`, or local equivalents.
...
The website demo and CLI run without an account. The website makes no network request outside the product origin. The CLI prints the temporary packet path so tests can inspect and verify it.
