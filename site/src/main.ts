...
function terms(): string {
  return shell(`<article class="legal"><p class="eyebrow">Terms · effective 2 September 2026</p><h1>Use the check as one safety signal</h1><p>LSP Readiness Check reports the tools it can observe. A passing result does not guarantee correct code or safe agent changes.</p><h2>Free CLI</h2><p>The open-source CLI is provided under the MIT License. You control where it runs and which test commands it executes.</p><h2>Private CI plan</h2><p>Private CI costs $49 per repository each month. It includes private required checks, policy templates, and readiness history.</p><p>Sociobot is the merchant of record. Billing, cancellations, and refunds follow the terms shown at hosted checkout. A refund or cancellation can end license access.</p><h2>Acceptable use</h2><p>Do not use the service to probe systems you do not own or have permission to test.</p><h2>Contact</h2><p>Questions can be sent to <a href="mailto:support@sociobot.in">support@sociobot.in</a>.</p></article>`);
}
